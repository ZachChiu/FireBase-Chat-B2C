    ...
    showList: function () {
      ...
      firebase.database().ref('chat').once("value").then(function (snapshot) {
        ...
        vm.chatlist.forEach((e, i) => {
          if (e.room.indexOf(vm.firmList[i].id) != -1) {
            e.name = vm.firmList[i].name;
            e.img = vm.firmList[i].img;
          }
        })
        vm.chatlist = vm.chatlist.sort(function (a, b) {
          return b.updatetime - a.updatetime
        })
        vm.company = vm.chatlist[0].firmId
        vm.showChat()
      });
    },
